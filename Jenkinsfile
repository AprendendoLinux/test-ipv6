                    ...
                    try {
                        withCredentials([usernamePassword(credentialsId: "${PORTAINER_CREDS_ID}", usernameVariable: 'PORTAINER_USER', passwordVariable: 'PORTAINER_PASS')]) {
                            sh '''
                                ...
                                # Criar nova stack
                                COMPOSE_CONTENT=$(cat "${WORKSPACE}/docker-compose.yml" | jq -sR .)
